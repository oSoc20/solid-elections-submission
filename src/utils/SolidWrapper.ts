...
export async function addTestForm(appContainer: TripleDocument, nom: string) {
    const personRef = appContainer.asRef() + '/testform.ttl';
    const personDoc = createDocument(personRef);
    const person = personDoc.addSubject();
    person.addRef(foaf.accountName, nom);
    return await personDoc.save([person]);
}
